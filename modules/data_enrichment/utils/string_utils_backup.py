...
class StringUtils:
    ...
    @staticmethod
    def generate_prompt(chemical_name: str, cas_number: Any) -> str:
        """
        ...
        prompt = f"""
# 🧪 化学品知识图谱数据查询指令 v4.0 - CAS号/流水号智能识别版
...
**重要：每个字段的数据后必须用大括号{{}}标明数据来源，格式：{{数据来源}}**

```json
{{
    "名称": "化学品标准中文名称 {{PubChem}}",
    "CAS号或流水号": "优先CAS号格式XXXX-XX-X，无CAS号时为流水号 {{PubChem}}",
    "编号类型说明": "标准CAS号/本地流水号/新分配CAS号 {{数据来源}}",
    "别名": "别名1; 别名2; 别名3 {{PubChem; ECHA}}",
    "分子式": "化学分子式 {{PubChem}}",
    "分子量": "数值 g/mol {{PubChem}}",
    "是否为危化品": "是/否 {{危险化学品目录}}",
    "浓度阈值": "毒理学数据详情 {{ECHA; GESTIS}}",
    "危害": "危害描述 {{SDS; NIOSH}}",
    "防范": "防护措施详情 {{SDS; OSHA}}",
    "危害处置": "应急处置详情 {{SDS; EPA}}",
    "用途": "用途详情 {{PubChem; 行业资料}}",
    "自然来源": "天然来源详情 {{科学文献; PubChem}}",
    "生产来源": "上游原料详情 {{供应商资料; 行业报告}}",
    "工业生产原料": "下游产品详情 {{供应商资料; 行业报告}}",
    "性质": "物理化学性质详情 {{PubChem; GESTIS}}"
}}
```
...
- 🏷️ **数据来源标注**: 每个字段的数据后必须用大括号{{}}标明具体数据来源，如{{PubChem}}、{{ECHA}}、{{GESTIS}}等，便于数据溯源和质量评估
